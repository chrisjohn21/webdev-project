document.addEventListener("DOMContentLoaded", function () {
    ...
    purchaseButton.addEventListener("click", function () {
      ...
      if (selectedProducts.length === 0) {
        ...
        // Display a form to enter name, address, and phone number
        Swal.fire({
...
// Generate a receipt with buyer's information, items, quantity, and total cost
const receiptText = `Buyer's Information:<br>Name: ${buyerName}<br>Address: ${buyerAddress}<br>Phone Number: ${buyerPhone}<br><br>Receipt:<br>${selectedProducts
    .map((product) => `${product.name} (Quantity: ${product.quantity}): ₱ ${(product.price * product.quantity).toFixed(2)}`)
